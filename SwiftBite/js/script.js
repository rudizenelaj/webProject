...
document.addEventListener('DOMContentLoaded', function () {
    ...
    orderTableBody.addEventListener('click', function (event) {
        ...
        if (target.classList.contains('delete-order-btn')) {
            ...
            const itemName = row.querySelector('td:first-child').innerText.split(' (')[0]; // Extract item name without quantity
    ...
    storeOrderBtn.addEventListener('click', function () {
        ...
        if (customerName && Object.keys(currentOrder).length > 0) {
            ...
            const totalOrderPrice = Object.values(currentOrder).reduce((total, item) => {
                return total + (parseFloat(item.price.replace('$', '')) * item.quantity);
            }, 0).toFixed(2);

            // Display the customer name, all ordered items, total quantity, and total price in the "All Orders" table
            const newRow = allOrdersTableBody.insertRow();
            newRow.innerHTML = `
                <td>${customerName}</td>
                <td>${getAllOrderedItems()}</td>
                <td>${totalOrderPrice}</td>
                <td><button class="complete-order-btn">Completed</button></td>
            `;

            // Clear the order history table, customer name input, and reset the current order
            orderTableBody.innerHTML = '';
    ...
    function getAllOrderedItems() {
        return Object.entries(currentOrder).map(([itemName, item]) => {
